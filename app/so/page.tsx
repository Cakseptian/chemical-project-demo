...
export default function AdminDashboard() {
  ...
  // Print per Part Number — handle bulk items in the filtered set
  const handlePrintByPartNo = (partNo: string, items: InventoryItem[]) => {
    const filtered = items.filter(i => i.part_number === partNo);
    const hasBulk = filtered.some(i => i.is_bulk);
    if (hasBulk && filtered.length === 1 && filtered[0].is_bulk) {
      // Single bulk item — show qty dialog
      setBulkPrintTarget({ item: filtered[0], count: 1 });
    } else {
      void printAllQR(filtered);
    }
  };

  // Print per Lokasi — handle bulk items in the filtered set
  const handlePrintByLocation = (location: string, items: InventoryItem[]) => {
    const filtered = items.filter(i => i.location === location);
    void printAllQR(filtered);
  };
          ...
          <div className="max-w-7xl mx-auto px-4 md:px-8 py-8">
            ...
            {activeTab === "inventory" && (
              <InventoryTab
                ...
                onPrintAllQR={(items) => void printAllQR(items)}
                onPrintByPartNo={handlePrintByPartNo}
                onPrintByLocation={handlePrintByLocation}
